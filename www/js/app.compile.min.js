...
angular.module("app.spinalforge.plugin").run(["spinalModelDictionary","$mdDialog",function(a,b){Autodesk.Viewing.theExtensionManager.registerExtension("PannelAnnotation",class{constructor(a,b){Autodesk.Viewing.Extension.call(this,a,b),this.viewer=a,a.registerContextMenuCallback("rightClickMenu",this.RightClick),this.panel=null}load(){return this.viewer.toolbar?this.createUI():(this.onToolbarCreatedBinded=this.onToolbarCreated.bind(this),this.viewer.addEventListener(av.TOOLBAR_CREATED_EVENT,this.onToolbarCreatedBinded)),!0}onToolbarCreated(){this.viewer.removeEventListener(av.TOOLBAR_CREATED_EVENT,this.onToolbarCreatedBinded),this.onToolbarCreatedBinded=null,this.createUI()}RightClick(a){a.push({title:"See container",target:()=>{var a=this.viewer.getSelection();if(1==a.length){document.getElementById("search").value=a[0];var b=document.getElementById("allList");this.getContainer(a[0],(a)=>{this.changeContainer(b,a)})}else alert("you must select 1 item")}})}unload(){return this.viewer.toolbar.removeControl(this.subToolbar),!0}createUI(){this.panel=new PanelClass(this.viewer,"Annotation"),this.initialize();var a=new Autodesk.Viewing.UI.Button("model-viewer");a.setIcon("fa-pencil"),a.onClick=()=>{this.panel.isVisible()?this.panel.setVisible(!1):this.panel.setVisible(!0)},a.addClass("model-viewer"),a.setToolTip("model viewer"),this.subToolbar=new Autodesk.Viewing.UI.ControlGroup("my-custom-view-toolbar"),this.subToolbar.addControl(a),this.viewer.toolbar.addControl(this.subToolbar)}initialize(){this.panel.initializeMoveHandlers(this.panel.container);var c=this.panel.container,d=(a)=>{var d=c,e=a;d.className+=" panelViewer";var f=document.createElement("div");f.className="spin row head";var g=document.createElement("div");g.className="col-sm-2 col-md-2 col-xs-2";var h=document.createElement("button");h.className="btn btn-primary btn-sm btn-block",h.innerHTML="<i class=\"fa fa-pencil-square-o\" aria-hidden=\"true\"></i> Note",h.onclick=()=>{var a=b.prompt().title("New Note").placeholder("Please enter the title").ariaLabel("New Note").clickOutsideToClose(!0).required(!0).ok("Create!").cancel("Cancel");b.show(a).then((a)=>{this.AddNote(a)},function(){})};var i=document.createElement("div");i.className="col-sm-2 col-md-2 col-xs-2";var j=document.createElement("button");j.className="btn btn-primary btn-sm btn-block",j.innerHTML="<i class=\"fa fa-eye\" aria-hidden=\"true\"></i> View",j.onclick=()=>{this.changeAllItemsColor(),this.changeAllIcon("fa-eye-slash",!0)};var k=document.createElement("div");k.className="col-sm-2 col-md-2 col-xs-2";var l=document.createElement("button");l.className="btn btn-primary btn-sm btn-block",l.innerHTML="<i class=\"fa fa-eye-slash\" aria-hidden=\"true\"></i> Hide",l.onclick=()=>{this.restoreAllItemsColor(),this.changeAllIcon("fa-eye",!1)};var m=document.createElement("div");m.className="list-group",m.id="allList";var n=document.createElement("div");n.className="spin input-group";var o=document.createElement("span");o.className="icon input-group-addon",o.innerHTML="<i class=\"fa fa-search\" aria-hidden=\"true\"></i>";var p=document.createElement("input");p.className="input form-control toolbar-search-box",p.type="number",p.id="search",p.placeholder="item id",p.onclick=function(){p.focus()},p.oninput=()=>{""==p.value?this.changeContainer(m,e):this.getContainer(parseInt(p.value),(a)=>{this.changeContainer(m,a)})},g.appendChild(h),i.appendChild(j),k.appendChild(l),f.appendChild(g),f.appendChild(i),f.appendChild(k),d.appendChild(f),n.appendChild(o),n.appendChild(p),d.appendChild(n),d.appendChild(m),e.bind(()=>{this.changeContainer(m,e)})};a.init().then((a)=>{a&&(a.annotationPlugin?a.annotationPlugin.load((a)=>{this.model=a,d(this.model)}):(this.model=new Lst,a.add_attr({annotationPlugin:new Ptr(this.model)}),d(this.model)))})}AddNote(a,b="#000000"){var c=this.model,d=newGUID(),e=new NoteModel;e.title.set(a),e.color.set(b),e.id.set(d);var f=c;f.push(e),c.mod_attr(f)}AddItems(a){var b,c,d=this.viewer.getSelection();return 0==d.length?void alert("No model selected !"):void this.viewer.model.getBulkProperties(d,{propFilter:["name"]},(d)=>{for(var e=this.model,f=0;f<e.length;f++)if(e[f].id==a){b=e[f].allObject,c=f;break}for(var g=0;g<d.length;g++)b.push(d[g]);e[c].allObject=b,console.log("notes",e)},function(){console.log("error")})}getContainer(a,b){var c=model,d=c.filter((b)=>0<b.allObject.filter((b)=>b.dbId.get()==a).length);b(d)}changeColorInHub(a,b){for(var c=this.model,d=0;d<c.length;d++)c[d].id==a&&c[d].color.set(b)}changeItemColor(a){for(var b,c=[],d=this.model,e=0;e<d.length;e++)if(d[e].id==a){b=d[e];for(var f=0;f<b.allObject.length;f++)c.push(b.allObject[f].dbId.get())}this.viewer.setColorMaterial(c,b.color.get(),b.id.get())}restoreColor(a){var b,c=[];console.log("id",a);for(var d=this.model,e=0;e<d.length;e++)if(d[e].id==a){b=d[e];for(var f=0;f<b.allObject.length;f++)c.push(b.allObject[f].dbId.get())}console.log(c),this.viewer.restoreColorMaterial(c,a)}changeAllItemsColor(){for(var a=[],b=this.model,c=0;c<b.length;c++){for(var d,e=[],f=0;f<b[c].allObject.length;f++)e.push(b[c].allObject[f].dbId.get());d=b[c].color.get(),a.push({ids:e,color:d,id:b[c].id})}this.viewer.colorAllMaterials(a)}restoreAllItemsColor(){for(var a,b=[],c=this.model,d=0;d<c.length;d++){a=[];for(var e=0;e<c[d].allObject.length;e++)a.push(c[d].allObject[e].dbId.get());b.push({ids:a,id:c[d].id})}this.viewer.restoreAllMaterialColor(b)}deleteNoteItem(a,b=null){var c=this.model;if(null!=b){for(let e=0;e<c.length;e++)if(c[e].id==a)for(var d=0;d<c[e].allObject.length;d++)c[e].allObject[d].dbId==b&&c[e].allObject.splice(d,1);}else c=c.filter((b)=>b.id!=a)}changeAllIcon(a,b){for(var c,d=this.model,e=0;e<d.length;e++)c=document.getElementsByClassName("show"+d[e].id)[0],c.innerHTML=`<i class="fa ${a}" aria-hidden="true"></i>`,c.show=b}changeContainer(a,c){if(a.innerHTML="",0<c.length)for(var d=0;d<c.length;d++){var e=c[d],f=e.title.get(),g=e.id.get(),h=e.color.get(),j=document.createElement("div");j.className="card";var k=document.createElement("div");k.className="card-header row",k.setAttribute("role","tab"),k.id="headerNote"+d,k.innerHTML=`<div class="col-sm-4 col-md-4 col-xs-4">
                      <a href="javascript:;" data-toggle="collapse" data-target="#notes${d}" aria-expanded="true" aria-controls="notes${d}">${f}</a>
                  </div>`;var l=document.createElement("div");l.className="col-sm-1 col-md-1 col-xs-1";var m=document.createElement("button");m.className="btn btn-sm",m.innerHTML="<i class=\"fa fa-plus\" aria-hidden=\"true\"></i>",m.id=g,m.onclick=()=>{this.AddItems(m.id)},l.appendChild(m),k.appendChild(l);var n=document.createElement("div");n.className="col-sm-1 col-md-1 col-xs-1";let b=document.createElement("input");b.className="btn btn-sm btn-block",b.setAttribute("type","color"),b.value=h,b.id=g,b.onchange=()=>{this.changeColorInHub(b.id,b.value)},n.appendChild(b),k.appendChild(n);var o=document.createElement("div");o.className="col-sm-1 col-md-1 col-xs-1";var p=document.createElement("button");p.className="btn btn-sm show"+g,p.innerHTML="<i class=\"fa fa-eye\" aria-hidden=\"true\"></i>",p.id=g,p.setAttribute("show",!1),p.onclick=()=>{!0==p.show?(this.restoreColor.call(this,p.id),p.show=!1,p.innerHTML="<i class=\"fa fa-eye\" aria-hidden=\"true\"></i>"):(this.changeItemColor.call(this,p.id),p.show=!0,p.innerHTML="<i class=\"fa fa-eye-slash\" aria-hidden=\"true\"></i>")},o.appendChild(p),k.appendChild(o);var q=document.createElement("div");q.id="notes"+d,q.className="collapse",q.setAttribute("role","tabpanel"),q.setAttribute("aria-labelledby","notes"+d),q.setAttribute("data-parent","#accordion");var r=document.createElement("div");r.className="card-body";var s=document.createElement("ul");s.className="list-group";var t,u=e.allObject.length;if(0<u)for(var v=0;v<u;v++){t=document.createElement("li"),t.className="list-group-item d-flex justify-content-between align-items-center",t.innerHTML=e.allObject[v].name;var i=document.createElement("button");i.className="btn badge",i.id=g+"/"+e.allObject[v].dbId,i.innerHTML="<i class=\"fa fa-trash-o\" aria-hidden=\"true\"></i>",i.onclick=()=>{var a=i.id.split("/");this.deleteNoteItem(a[0],a[1])},t.appendChild(i),s.appendChild(t)}else t=document.createElement("span"),t.innerText="No item",s.appendChild(t);r.appendChild(s),q.appendChild(r),j.appendChild(k),j.appendChild(q),a.appendChild(j)}else a.innerHTML="<h6>No item found</h6>"}})}]);
